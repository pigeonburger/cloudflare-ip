...
def read_keys_from_config_file(file):

	# Reading the keys from the cfauth.ini file
	config = configparser.ConfigParser()
	config.read(file)

	return config.get('tokens', 'zone_id'), config.get('tokens', 'bearer_token'), config.get('tokens', 'record_id')


def setup_log_file(file):
	# Setting up the logger (a file where it records all IP changes)
	logging.basicConfig(level=logging.INFO, filename=file, format='%(levelname)s :: %(message)s')


def handling_api_errors(currentip, ipcheck_status):
	# Handling any API errors (otherwise we'd be trying to change the IP to some random HTML)
	while ipcheck_status != STATUS_CODE_200:
		time.sleep(10)
		currentip = requests.get(API_URL)
		ipcheck_status = currentip.status_code

	return currentip, ipcheck_status


def main():
	setup_log_file('ipchanges.log')

	zone_id, bearer_token, record_id = read_keys_from_config_file('cfauth.ini')
	...
	CLOUDFLARE_URL = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records/{record_id}"

	while True:
		# Getting the initial data of your A Record
		a_record_url = requests.get(CLOUDFLARE_URL, headers=headers)
		...
		# Handling any API errors (otherwise we'd be trying to change the IP to some random HTML)
		currentip, ipcheck_status = handling_api_errors(currentip, ipcheck_status)
		...
		while currentactualip == current_set_ip:
			time.sleep(86400)  # Wait for 300 seconds (5 minutes)
			currentip = requests.get(API_URL)  # Then it checks if your IP is still the same or not.
			ipcheck_status = currentip.status_code
		...
		requests.patch(CLOUDFLARE_URL, headers=headers, data=json.dumps(payload))
